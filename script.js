document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadAllData() {
        try {
            // 使用 Promise.all 同時發送多個 fetch 請求
            const [paramsResponse, translationsResponse] = await Promise.all([
                fetch('parameters.json'),
                fetch('translations.json') // 新增讀取 translations.json
            ]);
            ...
            const i18nData = await translationsResponse.json(); // i18n 是 internationalization 的縮寫
            ...
            equivalentsData = i18nData.equivalentsData;


            // ★★★ 重要：等所有參數都載入後，才執行初始化設定 ★★★
            initializeApp();

        } catch (error) {
            console.error("無法載入或解析外部 JSON 檔案:", error);
            alert("錯誤：無法載入設定檔，請檢查 `parameters.json` 與 `translations.json` 檔案是否存在且格式正確。");
        }
    }
